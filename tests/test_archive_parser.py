...
@patch("src.archive_parser.zipfile.is_zipfile")
@patch("src.archive_parser.Path")
def test_unzip_valid_download(mock_Path, mock_is_zipfile, mock_Zipfile, mock_logger):
    ...
    mock_Zipfile_instance.extractall.return_value = extract_to
    mock_Zipfile.return_value = mock_Zipfile_instance
    unzip(zip_path=zip_path, extract_to=extract_to)

    mock_is_zipfile.assert_called_once_with("test.zip")
    mock_Zipfile.assert_called_once()
    mock_logger.info.assert_called_with("X Data Unzipped Successfully")
